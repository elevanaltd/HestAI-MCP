...
DISPATCH_FLOW::[
  ...
  STEP_8::"Agent needs another agent — calls dispatch_colleague, Workbench compiles fresh KVAEPH and spawns. CLAUDE_TARGET_CONTINUATION: for Claude-target dispatch_colleague continuation, the continuation uses Claude Code 2.1.77+ agentId + SendMessage within a live parent Claude panel (gated behind CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1). In-process only — parent panel must still be alive. For cross-process Claude continuation (after parent panel exit), fall back to claude --resume <session_id> via ContinuationStore. These are continuation mechanics; Vault identity + KVAEPH Position 3 + Alley-Oop remain authoritative. See HESTAI-ECOSYSTEM-LIGHTHOUSE.md §4 Anti-patterns AP1/AP2/AP3.",
...
WORKBENCH_STATUS::[
  ...
  NEXT::"Phase 3D (ContinuationStore TTL cleanup) and 3E (dispatch-chain UI, workbench #82) still NEXT per workbench's own PROJECT-CONTEXT.oct.md (v2.2, 2026-07-27) — not yet shipped."
]
VAULT_STATUS::[
...
HESTAI_CONTEXT_MCP_STATUS::[
  ...
  NEXT::"Phase 2 core integration (get_context @ KVAEPH Position 3) SHIPPED 2026-05-01 (workbench PRs #169/#176); submit_review consumer wiring (issue #30) deferred. Current focus: RFC #53 Gate C T6 migration/calibration (operator-involved, DUAL_KEY GO/NO-GO) — status not independently re-verified from this repo."
]
DEBATE_HALL_STATUS::[

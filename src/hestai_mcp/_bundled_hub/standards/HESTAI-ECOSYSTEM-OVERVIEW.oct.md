...
META:
  ...
  FORMAT::octave
§0::ARCHITECTURE_NOTE
DESCRIPTION::"This document describes the TARGET three-system Thick Client architecture. The workbench absorbs hestai-mcp and odyssean-anchor-mcp into a unified platform (Engine + Glass + Library). debate-hall-mcp and octave-mcp remain standalone. See docs/dream-team-architecture.md for the specification."
...
ECOSYSTEM::"Three systems that together provide agent identity, governance, deliberation, semantic compression, and a unified control panel"
§2::THE_THREE_SYSTEMS
SYSTEM_1::"HESTAI_WORKBENCH[REPO::elevanaltd/hestai-workbench, ROLE::The Unified Platform, OWNS::[agent_definitions, skills_library, anchor_ceremony, context_steward, session_lifecycle, governance_rules, agent_registry, worktree_orchestration, cli_spawning, terminal_ui, agent_dispatch, payload_compilation, pipeline_runner], ABSORBS::[hestai-mcp(library+governance+session_lifecycle), odyssean-anchor-mcp(anchor_ceremony+identity_binding)], ARCHITECTURE::Engine (Node/TS backend with payload compiler + dispatcher + pipeline runner + anchor validator) + Glass (React frontend with agent registry + session dashboard + workflow editor + dispatch chain visibility) + Library Manager (reads/writes .hestai-sys/ and .hestai/ directly), KEY_PROPERTY::Unified center. Knows WHO agents are, HOW they behave, WHICH provider runs them, WHERE sessions run. Single process, single debug target., DEPENDS_ON::[debate-hall-mcp, octave-mcp], DATA::[~/.hestai-workbench/library/ (canonical agent definitions, skills, standards), SQLite database (sessions, agent registry, UI state), git worktrees per session, .hestai-sys/ (written to worktrees at spawn — same pattern, workbench is writer), .hestai/ (project context, committed, read by workbench), .hestai/state/ (session state, written by workbench)]]"
...
HESTAI_MCP_ABSORPTION::[
  ...
  WHAT_REMAINS::"Repository continues as source-of-truth for library content during migration. Once workbench library is complete, hestai-mcp becomes archived.",
...
ODYSSEAN_ANCHOR_ABSORPTION::[
  ...
  EVIDENCE::"ADR-0275 decided merger. Original target was hestai-mcp. New target is workbench backend."
]
PAL_ELIMINATION::[
...
THREE_LOADING_PATHS::[
  FORMAL::"Full 5-stage anchor ceremony. For T3-T4 high-stakes work. Agent proves comprehension.",
  COLLEAGUE::"Workbench pre-compiles payload. Agent gets lightweight verification. Default for T1-T2.",
  DEBATE::"Cognition + identity injection only. For deliberation sessions."
]
...
CLEAR_SEPARATIONS::[
  ...
  "Agent library — ONE canonical source in workbench. Not duplicated elsewhere."
]
§6::ON_DISK_STRUCTURE
...
DIRECTORIES::[
  ...
  ".hestai/state/ — session state, written by workbench, displayed in Glass UI"
]
...
§8::KEY_PRINCIPLES
P1::"The workbench is the unified platform. Identity, governance, execution, and UI under one roof."
P2::"One canonical source for agent definitions and skills. In the workbench library."
P3::"Pre-compiled payloads by default. Full ceremony only when tier demands it."

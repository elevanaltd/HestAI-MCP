...
SYSTEM_3::"HESTAI_CONTEXT_MCP[REPO::\"elevanaltd/hestai-context-mcp (IMPLEMENTED — Phase 1 complete 2026-04-17)\", ROLE::The Memory and Environment, OWNS::[clock_in(session_creation+focus_resolution+AI_synthesis_currently_null+focus_conflict_detection), clock_out(TranscriptParser_ABC+ClaudeTranscriptParser_adapter+credential_redaction+OCTAVE_compression+learnings_indexing), get_context(read_only_context_synthesis), ContextSteward(dynamic_PhaseConstraints), submit_review(structured_review_verdicts+CI_gate+8_roles+dry_run+SHA_pinning), dotHestai_state_management, product_north_star_injection_planned_phase_3], TRANSPORT::stdio_JSON_RPC, DEPENDS_ON::[nothing_at_runtime], TESTS::\"361 passing, 89 percent coverage\", KEY_PROPERTY::\"LOW volatility. Python codebase, 361 tests, 89 percent coverage at Phase 1 close. Stdio MCP transport (subprocess not daemon). Survives Workbench rebuilds untouched. Terminal parity automatic.\", ADAPTER_PATTERN::\"clock_out redesigned with TranscriptParser ABC. ClaudeTranscriptParser implemented. Codex/Gemini/Goose adapters pending Phase 2+.\", KNOWN_GAPS::[clock_in_AI_synthesis_returns_null_functional_regression_vs_legacy, non_claude_transcript_adapters_pending]]"
...
§3::LEGACY_SYSTEMS
HESTAI_MCP::[STATUS::legacy_maintenance_mode_v2.1.0_1033_tests,REPO::"elevanaltd/HestAI-MCP",DISPOSITION::[
...
],NOT_BEING_ABSORBED::"ADR-0353 resolved: hestai-mcp is NOT being absorbed into the Workbench. Governance engine logic has been harvested into the NEW repo (hestai-context-mcp, Phase 1 complete 2026-04-17). Legacy stays for comparison.",EVIDENCE::"1033 tests, v2.1.0, maintenance mode. Proven patterns inform the harvest.",OPEN_QUESTIONS::[measurable_deprecation_criteria_not_yet_defined_flagged_for_human_decision,PyPI_publication_plan_for_hestai_context_mcp_flagged_for_human_decision]]
ODYSSEAN_ANCHOR_MCP::[
...
HESTAI_CONTEXT_MCP_STATUS::[STATUS::"Phase 1 COMPLETE (2026-04-17). elevanaltd/hestai-context-mcp repo shipped: 4 tools (clock_in, clock_out, get_context, submit_review), 361 tests, 89 percent coverage, all quality gates green.",ADAPTER_PATTERN::"clock_out redesigned with TranscriptParser ABC + ClaudeTranscriptParser (Claude adapter implemented; Codex/Gemini/Goose adapters pending).",KNOWN_GAP::"clock_in AI synthesis currently returns null — functional regression vs legacy. Must be resolved before A/B parity claim.",NEXT::"Phase 2 — workbench Payload Compiler integration via stdio at KVAEPH Position 3. BLOCKED on workbench Step 3B Phase 2 completion."]
DEBATE_HALL_STATUS::[
...
HESTAI_MCP_LEGACY_STATUS::[STATUS::"operational, v2.1.0, 1033 tests, maintenance mode",DISPOSITION::"Stays for A/B comparison. Deprecation criteria not yet defined (OPEN QUESTION flagged for human decision). Eventually deprecated after new system proven and clock_in AI synthesis regression resolved."]
§8::KEY_PRINCIPLES

...
ENFORCEMENT::"Every octave_write ∨ octave_validate call → inspect status ∧ errors ∧ validation_status ∧ corrections ∧ warnings before claiming VALIDATED"
...
NOT_A_TOOL::"octave_fmt — planned under ADR-0006 SR3-T2, not registered in 1.15.0; do not call it"
OCTAVE_WRITE_MODES:
...
OCTAVE_VALIDATE::"content= ∨ file_path= (exclusive); profile STRICT ∨ STANDARD ∨ LENIENT ∨ ULTRA; schema required; compact=true → counts only; diff_only=true → diff not canonical body"
§3::RECEIPT_GATES
RESULT_FIELDS::[
  status,
  errors,
...
EMPTY_WARNINGS_RULE::"warnings:[] ≠ untouched — read corrections[] to learn what changed; only corrections:[] ∧ warnings:[] together mean the bytes were already canonical"
TRIAGE:
  ...
  ADVISORY::"form debt, non-blocking — W_ANNOTATION_TOO_LONG, W_SNAKE_CASE_BLOB, W_INLINE_ARRAY_ROOT, W_FLAT_PREFIX_SCALAR, W_CONSTRUCTOR_MISUSE → remediate per §6 when already amending the record"
...
NOOP_INVARIANT::"content identical to target bytes → true no-op: no normalisation, corrections:[] (ADR-0006 SR1-T4)"
...
MUST::[
  ...
  "read status ∧ errors ∧ validation_status ∧ corrections ∧ warnings — triage DISCARDING ∨ ADVISORY ∨ BENIGN",
  "changes mode: MERGE to keep siblings; bare dict = FULL REPLACE; nested ∨ new key → content= preserve",
  "W_ANNOTATION_TOO_LONG ∨ W_SNAKE_CASE_BLOB → remediate JIT, only on the record already being amended"
]
...
P6::"no-op invariant on identical bytes → CHANGELOG 1.12.0 SR1-T4 (#407)"

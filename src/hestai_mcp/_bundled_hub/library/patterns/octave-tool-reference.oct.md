...
RECEIPT_GATE::"status:success ∧ errors:[] ∧ validation_status ≠ INVALID ∧ warnings:[] ∧ every corrections[] entry AND every repairs[] entry triaged per §3::TRIAGE (repairs[] subtypes mapped by §3::ALIAS; unmapped subtype → DISCARDING when authoring, surfaced-not-classified at retry-capped gates) as BENIGN ∨ consciously accepted — a DISCARDING entry on either side fails the gate even when validation_status says VALIDATED"
§4::CHANGES_MODE

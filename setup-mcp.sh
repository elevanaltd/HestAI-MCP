...
#
# Configures the HestAI MCP server for use with various AI clients:
# - Claude Desktop
# - Claude Code CLI
# - OpenAI Codex CLI
# - Google Gemini CLI
...
#   ./setup-mcp.sh --uninstall        # Remove HestAI from all clients
# ============================================================================
...
# Configuration
readonly MCP_SERVER_NAME="hestai"
...
get_server_path() {
    local script_dir=$(get_script_dir)
    echo "$script_dir/src/hestai_mcp/mcp/server.py"
}
...
ensure_venv_exists() {
    ...
    if ! "$venv_python" -c "import hestai_mcp" 2>/dev/null; then
        print_info "Installing hestai-mcp package..."

        # Use uv pip if uv is available, otherwise use regular pip
        if command -v uv &> /dev/null; then
            uv pip install -e "$script_dir" --quiet
            if [[ $? -ne 0 ]]; then
                print_error "Failed to install hestai-mcp package with uv"
                return 1
            fi
        else
            "$venv_python" -m pip install -e "$script_dir" --quiet
            if [[ $? -ne 0 ]]; then
                print_error "Failed to install hestai-mcp package with pip"
                return 1
            fi
        fi
...
uninstall_all() {
    print_header "Uninstalling HestAI MCP Server"
    ...
    print_success "HestAI MCP Server uninstalled from all clients"
}
...
show_config() {
    ...
    print_header "HestAI MCP Server Configuration"
...
interactive_setup() {
    print_header "HestAI MCP Server Setup"

    echo ""
    echo "This script will configure the HestAI MCP server for your AI clients."
    echo ""
    echo "HestAI MCP provides:"
    echo "  - clock_in: Session registration with AI-powered context synthesis"
    echo "  - clock_out: Session archival and transcript extraction"
    echo "  - anchor_submit: Agent identity validation (Odyssean binding)"
    echo ""
...
Options:
  ...
  --uninstall        Remove HestAI from all clients
...
Tools Provided:
  - clock_in: Start agent session with AI-powered context synthesis
  - clock_out: Archive session and extract transcript
  - anchor_submit: Validate agent identity (Odyssean binding)

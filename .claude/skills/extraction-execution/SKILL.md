...
GATE_EXECUTION::BLOCKING::[
  ...
  GATE_1::TYPECHECK::[
    pnpm[turbo_typecheck --filter=target]>.hestai/validation/typecheck.txt,
    EXIT≠0→HALT["❌TypeCheck_FAILED"+show_errors+exit_1]
  ],

  GATE_2::LINT::[
    pnpm[turbo_lint --filter=target]>.hestai/validation/lint.txt,
    EXIT≠0→HALT["❌Lint_FAILED"+show_errors+exit_1]
  ],

  GATE_3::TEST::[
    pnpm[turbo_test --filter=target]>.hestai/validation/test.txt,
